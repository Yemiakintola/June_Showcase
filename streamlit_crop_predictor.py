...
import requests
import streamlit as st # Assuming this function is part of a Streamlit app where st is available
...
def get_soil_ph(lat, lon):
    ...
    try:
        ...
        # Debugging: Print the full response data structure if needed
        # import json
        # st.write("SoilGrids API Response:")
        # st.json(data)

        # Safely navigate the nested JSON structure step-by-step
        properties = data.get('properties', {})
        if not properties:
            st.warning("Soil pH data: 'properties' key not found or is empty in API response.")
            return None

        phh2o = properties.get('phh2o', {})
        if not phh2o:
            st.warning("Soil pH data: 'phh2o' key not found or is empty.")
            return None

        layers = phh2o.get('layers')
        if not layers or not isinstance(layers, list) or not layers:
             st.warning("Soil pH data: 'layers' not found, not a list, or is empty.")
             return None

        # Access the first layer (assuming the first one contains the relevant data)
        first_layer = layers[0]
        if not isinstance(first_layer, dict):
             st.warning("Soil pH data: First layer is not a dictionary.")
             return None

        depths = first_layer.get('depths')
        if not depths or not isinstance(depths, list) or not depths:
             st.warning("Soil pH data: 'depths' not found, not a list, or is empty in the first layer.")
             return None

        # Access the first depth entry (assuming the first one is relevant)
        first_depth = depths[0]
        if not isinstance(first_depth, dict):
             st.warning("Soil pH data: First depth entry is not a dictionary.")
             return None


        values = first_depth.get('values', {})
        if not values:
             st.warning("Soil pH data: 'values' key not found or is empty in the first depth entry.")
             return None

        ph_value = values.get('mean')

        if ph_value is not None:
            # Check if ph_value is a number before performing calculation
            if isinstance(ph_value, (int, float)):
                return round(ph_value / 10, 1)  # ISRIC returns pH * 10
            else:
                 st.warning(f"Soil pH data: 'mean' value is not a number: {ph_value}")
                 return None
        else:
            st.warning("Soil pH data: 'mean' value not found in API response.")
            return None

    except requests.exceptions.RequestException as e:
        st.error(f"Error fetching soil pH data: {e}")
        return None
    except IndexError:
         st.warning("Soil pH data: Unexpected list structure (IndexError).")
         # You might want to print the data structure here to debug further
         # import json
         # st.write("SoilGrids API Response causing IndexError:")
         # st.json(data)
         return None
    except Exception as e:
         st.error(f"An unexpected error occurred while processing soil pH data: {e}")
         return None
