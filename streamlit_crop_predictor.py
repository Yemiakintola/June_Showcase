...
def get_soil_ph(lat, lon):
    ...
    try:
        ...
        data = res.json()
        # Safely access nested keys
        ph_value = data.get('properties', {}).get('phh2o', {}).get('layers', [{}])[0].get('depths', [{}])[0].get('values', {}).get('mean')
        if ph_value is not None:
            return round(ph_value / 10, 1)  # ISRIC returns pH * 10
        else:
            st.warning("Soil pH data not available for this location.")
            return None
    except requests.exceptions.RequestException as e:
        st.error(f"Error fetching soil pH data: {e}")
        return None
    except IndexError:
         st.warning("Soil pH data structure unexpected.")
         return None
    except KeyError as e:
         st.warning(f"Error parsing soil pH data: Missing key {e}")
         return None
